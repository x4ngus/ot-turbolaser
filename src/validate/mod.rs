...
//! scan or a runt); `dragos_csv` scores a sensor export for how many planned
...
/// bands and/or score a Dragos CSV export for union-rate against the plan. Exits
...
pub fn cmd_verify(args: &VerifyArgs) -> i32 {
    ...
    // Score a Dragos export against the plan.
    if let Some(path) = &args.csv {
        did_something = true;
        match std::fs::read_to_string(path) {
            Ok(text) => match dragos_csv::analyze(&text, &ledger) {
    ...
    if !did_something {
        eprintln!(
            "nothing to verify: pass --csv <export> to score a Dragos export, or --pcap <file> \
...
fn print_union(path: &Path, r: &dragos_csv::UnionReport, json: bool) {
    if json {
        let val = serde_json::json!({
