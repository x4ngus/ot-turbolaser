...
fn mac_for(mac_map: &HashMap<u32, [u8; 6]>, seed: u64, new_ip: u32) -> [u8; 6] {
    mac_map
        .get(&new_ip)
        .copied()
        .unwrap_or_else(|| host_mac(seed, new_ip))
}

/// A believable, stable MAC for a generic capture host: an IT/PC/server vendor
/// OUI (so the sensor names it instead of seeing an unknown OUI) with the low
/// three bytes derived per-IP, so each host keeps one distinct, stable address.
/// Globally administered by construction (real OUIs are). Used for BOTH the
...
/// (multicast bit clear). This matters: a passive sensor ignores
...
mod tests {
    ...
    fn apply_host_map_remaps_arp_addresses_and_macs() {
        ...
        let sender_mac = host_mac(seed, new_spa);
        let target_mac = host_mac(seed, new_tpa);
        assert_eq!(
            &d[28..32],
            &[10, 9, 0, 5],
    ...
    #[test]
    fn host_mac_uses_a_real_it_oui_and_is_stable() {
        let seed = 9;
        let ip = u32::from(Ipv4Addr::new(10, 5, 0, 7));
        let m = host_mac(seed, ip);
        assert_eq!(m, host_mac(seed, ip), "stable for (seed, ip)");
        assert_ne!(m, host_mac(seed, ip + 1), "distinct per IP");
        assert_eq!(m[0] & 0x02, 0, "globally administered (sensor binds it)");
        assert!(
            crate::oui::OuiDb::embedded()
                .vendor_of_prefix([m[0], m[1], m[2]])
                .is_some(),
            "the host's OUI resolves to a known IT vendor, not an unknown prefix"
        );
    }
    ...
    fn apply_host_map_rewrites_ipv4_macs_and_preserves_broadcast_dst() {
        ...
        assert_eq!(
            ...
            &host_mac(seed, new_src),
            "src MAC is the sender's host MAC (IT-vendor OUI)"
        );
        assert_eq!(
            &uni.packets[0].data[0..6],
            &host_mac(seed, new_dst),
            "dst MAC is the peer's host MAC (IT-vendor OUI)"
        );
