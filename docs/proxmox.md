...
isolated segment with no route to anything real. A quick copy-and-paste path
first, then the full reference (Dell R740 layout, the sensor VM, the host SPAN
mirror, OVS, and troubleshooting).
...
the synth burst against the reference OT ARP bands and scores a passive-sensor export
...
classified split will not re-union, so clear the sensor's inventory (or re-seed the
...
```
  ...
  +----------------------------------------------------------------------+
  |  ot-turbolaser CT 910               passive sensor VM 200            |
  |  +--------------------+             +------------------------------+  |
...
- On the sensor, confirm the assets categorise by vendor (no MAC-only or IP-only
